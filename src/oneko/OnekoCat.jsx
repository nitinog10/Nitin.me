...
/* Sprite sheet coordinates, straight from oneko.js — each pair is a 32px tile. */
const spriteSets = {
  idle: [[-3, -3]],
  alert: [[-7, -3]],
  scratchSelf: [
...
export default function OnekoCat({
  ...
  const setSprite = (name, frameNo) => {
    if (!catRef.current) return;
    const set = spriteSets[name];
    if (!set) return;
    const sprite = set[frameNo % set.length];
  ...
  // Restore where this pet was left last visit
  useEffect(() => {
    ...
    if (savedNeko) {
      try {
        ...
        /* corrupt entry — just start fresh */
      }
    } else {
    ...
    const handleBeforeUnload = () => {
      if (enabled && catRef.current) {
        ...
        try {
          ...
          /* private mode — the pet just starts from its corner next time */
        }
      }
    };
